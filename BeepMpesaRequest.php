...
function logMpesaRequest() {
    ...
    // $dbLink= connectDB();
    $query = "INSERT INTO requestLog(amount,MSISDN,transactionTypeID,payerTransactionID,invoiceNumber,dateCreated) VALUES('$amount','$msisdn',3,'$payerTrx','$invoiceNo','$date')";
    $sql = insertSQL($query);
    ...
    if ($sql) {
        ...
        if (!$client->query('BEEP.postPayment', $payload)) {
            ...
            return "Your request to MPESA Transfer has failed .Please try again";
        } else {
            $response = $client->getResponse();
            return updateMpesa($response);
        }
    } else {
...
function updateMpesa($response) {
    ...
    if ($response['statusCode'] == 139) {
        ...
        $sql = updateSQL($query);
        if ($sql > 0) {
            return "Your request to MPESA transfer has been received successfully .Please wait";
        } else {
            return "Your request to MPESA has failed .Please try again";
        }
    } else {  ////if beep returned an error
        return "Your request to MPESA has failed .Please try again." . $response['statusDescription'];
    }
}
