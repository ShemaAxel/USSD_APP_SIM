...
function postAClick(e, targetButtonID){
...
function set_time(time_object){
	...
	{
		document.getElementById("debug-div").innerHTML += "\nTIME DEBUG:" + htmlEntities(time_object['debug']);
	}
	sajax_debug_text = "";
...
function go_paybill(){
    document.getElementById("display-screen").innerHTML = loading;
    x_pay_bill("Pay Bill", do_display);
}

function go_b2ctransfer(){
    document.getElementById("display-screen").innerHTML = loading;
    x_b2ctransfer("Cash Transfer", do_display);
}

function go_checkBalance(){
    document.getElementById("display-screen").innerHTML = loading;
    x_checkBalance("Balance", do_display);
}

function go_newmenus(){
    document.getElementById("display-screen").innerHTML = loading;
    x_newmenus("Menu Options", do_display);
}

function go_topup(){
    document.getElementById("display-screen").innerHTML = loading;
    x_topup("Topup Airtime", do_display);
}
...
function ussd_do_post_BillRequest(mobile_number, amount, account_number){
    previouse_func = "go_paybill";
	document.getElementById("display-screen").innerHTML = loading;
	x_postBillRequest(mobile_number.value, amount.value, account_number.value, do_display);
}

function ussd_do_post_airtime(amount){
	document.getElementById("display-screen").innerHTML = loading;
	x_airtime_request(amount.value, do_display);
}

function ussd_do_post_b2c(amount){
	document.getElementById("display-screen").innerHTML = loading;
	x_b2cRequest(amount.value, do_display);
}
