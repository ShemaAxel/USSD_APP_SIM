...
class VirtueGhana extends DynamicMenuController {
    
    /**
     * @var Min loan amount
     */
    private $minLoanAmount = '500';
    
    /**
     * @var $maxLoanAmount
     */
    private $maxLoanAmount = '1000';
    ...
    public function processLanguage($input) {
        if (!is_numeric($input) || !in_array($input, array('1', '2'))) {
            $this->displayText[] = 'Invalid option, Choose Language';
            $this->displayText[] = '1. English';
            //$this->displayText[] = '2. kinyarwanda';
            $this->nextFunction = 'processLanguage';
            $this->sessionState = 'CONTINUE';
            ...
            switch ($input) {
                case 2:
                    $this->saveSessionVar('language', 'en_gh');
                    break;
                case 1:
                default:
                    $this->saveSessionVar('language', 'en_gh');
                ...
                case 1:
                    ...
                    if($glAccount) {
                        if($glAccount->loanID == '' || $glAccount == NULL) {
                            ...
                            $message = str_replace('^MIN_AMOUNT^', $this->minLoanAmount, $message);
                            $message = str_replace('^MAX_AMOUNT^', $this->maxLoanAmount, $message);
                            $this->displayText[] = $message;
                            ...
                            //Ask them to repay their existing loan
                ...
                case 2:
                    ...
                    if($pendingLoanRequest) {                                                
                        ...
                        if ($glAccount) {
                            if ($glAccount->loanID == '' || $glAccount == NULL) {
                                //the gl account does not have a loan
                                $message = $lang['min-max-amount'];
                                //$message = str_replace('^MAX^', $message, $this->maxLoanAmount);
                                //$message = str_replace('^MIN^', $message, $this->minLoanAmount);
                                $this->displayText[] = $message;
                                $this->displayText[] = $lang['main-menu'];
   ...
   //Process Loan Amount
    public function processLoanAmount($input) {
        ...
        if($input == '00') {
            ...
            $this->sessionState = 'CONTINUE';
            $this->nextFunction = 'serviceOptions';
        } elseif($input < $this->minLoanAmount || $input > $this->maxLoanAmount) {
            $this->displayText[] = $lang['min-max-amount'];            
            $this->displayText[] = $lang['exit'];
            $this->nextFunction = 'processLoanAmount';
            $this->sessionState = 'CONTINUE';
        } else {
...
//processLoanOptions
    ...
    public function processLoanRequest($input) {
        ...
        if ($input == "") {
            ...
            if ($input == '00') {
                $this->displayText[] = 'Thank you for choosing Virtue Ghana. Bye';
                $this->sessionState = 'END';
                exit();
            } elseif ($input == '99') {
