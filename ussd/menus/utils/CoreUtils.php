...
class CoreUtils {
    ...
    /**
     * Function to validate the MSISDN against the prefixes
     *
     */
    public static function isValidMobileNo($userNumber)
    {
        $MSISDN = str_replace(array("  ", " ", "+"), "", $userNumber);

        $query = "SELECT COUNT(*) FROM networks WHERE ? REGEXP numberRuleRegex and numberRuleRegex != '' AND numberRuleRegex is not null";

        $dbUtils = new DbUtils();
        $response = $dbUtils->getCount($query, array($MSISDN));

        if ($response['SUCCESS'] && isset($response['DATA']['COUNT']) && $response['DATA']['COUNT'] > 0)
            return true;
        return false;
    }
}
