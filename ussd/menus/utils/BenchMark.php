...
class BenchMark
{
    ...
    public function elapsed_time($method, $uniqueid) {
        if (!isset($this->limit[$method][$uniqueid]['start']) || empty($this->limit[$method][$uniqueid]['start'])) {
            ...
            return sprintf("%.4f", $this->limit [$method][$uniqueid]['end'] - $this->limit[$method][$uniqueid]['start']);
        }
    }
    ...
    public function logFunctionTAT($method, $uniqueid = -1) {
        try {
            $this->log->info(
                ...
                $method . " - FUNCTION RUN TIME: " . $this->elapsed_time($method, $uniqueid)
    ...
    public function logTotalTAT($method, $uniqueid = -1) {
        try {
            $this->log->info(
                Config::TAT_INFO, $uniqueid,
                $method . " - TOTAL RUN TIME: " . $this->elapsed_time($method, $uniqueid)
                . " seconds | elapsed memory: " . $this->memory_usage()
            );
        } catch (Exception $ex) {
            $this->log->error(
    ...
    public function logInvocationTAT($method, $uniqueid = -1) {
        try {
            $this->log->info(
                Config::TAT_INFO, $uniqueid,
                $method . " - TOTAL INVOCATION TIME: " . $this->elapsed_time($method, $uniqueid)
                . " seconds | elapsed memory: " . $this->memory_usage()
            );
        } catch (Exception $ex) {
            $this->log->error(
